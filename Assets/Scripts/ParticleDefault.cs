...
public class ParticleDefault : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if ((timer - Time.time) >= 0.0) {
			...
			this.light.color = new UnityEngine.Color((190.0f + sinData / (max_shot - min_shot) * 65.0f) / 255.0f, 190.0f / 255.0f, (255.0f - sinData / (max_shot - min_shot) * 65.0f) / 255.0f);

			//for particle 
			this.particleSystem.startColor = new UnityEngine.Color((190.0f + sinData / (max_shot - min_shot) * 65.0f) / 255.0f, 190.0f / 255.0f, (255.0f - sinData / (max_shot - min_shot) * 65.0f) / 255.0f);
